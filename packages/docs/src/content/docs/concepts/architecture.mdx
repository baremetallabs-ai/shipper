...
Desktop distribution is currently unsigned macOS arm64 only; see [Desktop](/guides/desktop/) for
...
`shipper_unlock`. Direct in-process core helpers include `shipper_docs_search`, `shipper_docs_get`,
and `shipper_reset` via `executeReset`. Workflow-changing agent work such as `shipper_advance`,
experimental `shipper_groom`, `shipper_create_issue`, `shipper_unblock`, and `shipper_merge` spawns
headless `shipper` CLI runs. The [MCP reference](/reference/mcp/) owns the per-tool surface, while
[MCP setup](/guides/mcp-setup/) owns client setup.
