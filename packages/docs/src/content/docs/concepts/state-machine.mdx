---
title: State Machine
description: GitHub labels, transitions, rollback paths, and auto-ship ordering for Shipper workflow state.
---
...
If an issue is missing this exactly-one workflow-label invariant, see
...
`shipper new` is Shipper-owned after draft validation: the agent writes draft files under
`.shipper/output/`, then Shipper creates the GitHub issue and applies `shipper:new`. Grooming still
performs issue-body edits/comments and applies the initial `shipper:groomed` transition through its
post-flight flow. The result-file transition contract owns the downstream workflow stages.
...
For recovery from failed issues, reject loops, or transition-cap exhaustion, see
...
For symptom-first reset guidance, see
[failed issues and rollback loops](/troubleshooting/common-errors/#failed-issues-and-rollback-loops).
...
For stale lock recovery, see [stale locks](/troubleshooting/common-errors/#stale-locks).
...
For the short recovery pointer for blocked issues, see
[failed issues and rollback loops](/troubleshooting/common-errors/#failed-issues-and-rollback-loops).
