...
- [shipper_create_issue](./shipper_create_issue) — Create a new GitHub issue from a plain-text request. Spawns `shipper new <request> --mode headless`; the agent drafts under `.shipper/output/`, then Shipper validates, creates the issue, applies `shipper:new`, and records the created issue identity. Requires a non-empty request.
