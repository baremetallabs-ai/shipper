...
issue argument and, when omitted, auto-select the first eligible issue. `pr open` accepts an
optional issue argument and, when omitted, auto-selects the first eligible issue. `pr review` and
`pr remediate` accept an optional PR number or URL and, when omitted, auto-select the first eligible
PR; they can also accept an issue reference through linked PR resolution.
