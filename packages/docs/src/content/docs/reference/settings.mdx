...
| `installCommand`              | unset                                                                   | Shell command used for the advisory dependency-install step in new worktrees. Runs through `/bin/sh -c` (POSIX shell). On Linux `/bin/sh` is typically dash; bash-specific syntax (`[[ ]]`, arrays) will fail — wrap in `bash -c "..."` if needed.                                                                                                                                                                                                      |
...
rerun `shipper init` to refresh the runtime fingerprint. The
