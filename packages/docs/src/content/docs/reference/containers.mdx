...
`GITHUB_TOKEN` works identically. In GitHub Actions, `GITHUB_TOKEN` is the default token name, so pass
it into the step or container environment where Shipper runs.
