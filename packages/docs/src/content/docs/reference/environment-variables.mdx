...
build-time, Electron, cache, and worktree implementation variables are intentionally excluded except
for the Shipper-set MCP bridge handoff documented below.
...
| Read by            | Core prompt runner bridge gate and Claude bridge artifact installation.                         |
| Default when unset | Bridge disabled; headless Claude runs outside the streamed MCP path disallow `AskUserQuestion`. |
