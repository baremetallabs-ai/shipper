...
| Read by            | Shipper CLI/MCP GitHub auth preflight in core.                                                                                                  |
| Default when unset | Shipper relies on `gh auth login` or the existing GitHub CLI authentication available to the process.                                           |
| Accepted values    | Any non-empty GitHub token string with the repository access needed for the Shipper operation; Shipper only checks that the value is non-empty. |
...
| Read by            | The same Shipper CLI/MCP GitHub auth preflight after `GH_TOKEN`.                                      |
...
| Field              | Value                                                                                                         |
| ------------------ | ------------------------------------------------------------------------------------------------------------- |
| Set by             | Shipper for hooks when issue context exists.                                                                  |
| Read by            | User-authored hook scripts.                                                                                   |
| Default when unset | Outside Shipper hook execution, Shipper does not provide it; in hooks without issue context, it may be empty. |
