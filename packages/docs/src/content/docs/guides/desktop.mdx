...
Use `Logs` from an eligible queue row to open the background log viewer. Queued rows expose `Logs`
only after they have output to show; otherwise they have no log content until they start. Running
commands stream output into the viewer as it arrives. Finished commands show the buffered output
retained for that row. `New` rows use the same viewer after the desktop can load their JSONL log
file.
