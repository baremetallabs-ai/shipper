...
`Succeeded`, `Failed`, or `Paused` rows. `Clear finished` removes every non-active row and leaves
...
Use `Logs` from an eligible queue row to open the background log viewer. Running commands stream
output into the viewer as it arrives. Finished commands show the buffered output retained for that
row. `New` rows use the same viewer after the desktop can load their JSONL log file.
...
`Auto-merge` is saved in desktop app config per repository. When enabled for the selected repository,
desktop passes merge behavior into `Ship` commands so ready PRs can enter the merge flow.
