...
**Note:** The issue comment for this stage is normally a one-liner. If you have nothing to report, keep using the inline `gh issue comment <ISSUE> --body "PR opened: <PR_URL>"` command above. If you have agent feedback to append, create `./.shipper/tmp/pr-open-comment-<number>.md` with:
...
Then post it with `gh issue comment <ISSUE> --body-file ./.shipper/tmp/pr-open-comment-<number>.md` instead of inline `--body`.
