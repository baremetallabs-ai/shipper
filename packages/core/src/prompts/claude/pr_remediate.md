...
- Before doing anything, read these pass artifacts that Shipper created for you:
...
- If appended user input contains merge-conflict context, resolve those files carefully, stage them with `git add`, create a commit only if the repository requires it during conflict resolution, and stop there. Do not attempt transport commands yourself.

## Phase 1: Orient

1. Read the issue and PR context from the appended message.
2. Read the four `.shipper/input/` files.
