...
export function useRepos({
  ...
  const resetRepoState = useEffectEvent(() => {
  ...
  const handleToggleAutoMerge = useEffectEvent(async (repo: string) => {
    ...
    try {
      ...
      const message = toErrorMessage(error);
      pipelineBridgeRef.current?.setFetchError(`Failed to save auto-merge preference: ${message}`);
    } finally {
  ...
  const handleAddRepo = useEffectEvent(async (repo: string) => {
  ...
  const handleSwitchRepo = useEffectEvent(async (repo: string) => {
    if (repo === activeRepo) {
      return;
    }
  ...
  const handleReorderRepos = useEffectEvent(async (nextRepos: string[]) => {
    if (
      repos.length < 2 ||
  ...
  const handleCloseRepo = useEffectEvent(async (repo: string) => {
