...
2. Verify required labels exist (`shipper:designed`, `shipper:planned`): run `gh label list --search "shipper:" --json name -q '.[].name'`. If missing: tell the user to run `shipper init`, then stop.
...
1. [Specific, testable check — e.g. "Run `npm test` and confirm all existing tests pass"]
2. [Another check tied to acceptance criteria]
3. [...]
