...
2. Verify required labels exist (`shipper:new`, `shipper:groomed`): run `gh label list --search "shipper:" --json name -q '.[].name'`. If missing: tell the user to run `shipper init`, then stop.
